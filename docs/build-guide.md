...
此命令会生成：
- `win-unpacked/` — 解压即用目录
- `textdiff-x.x.x-setup.exe` — NSIS 安装包（含 x64 + ia32）
- `textdiff-x.x.x-setup-x64.exe` — NSIS 安装包（仅 x64）
- `textdiff-x.x.x-setup-ia32.exe` — NSIS 安装包（仅 ia32）
- `textdiff-x.x.x-portable-x64.exe` — 便携版（免安装）
...
将 `package.json` 的 `version` 改回 `1.0.0`（或保留新版本号，视情况而定）。
...
```
dist/v1.1.0/
├── win-unpacked/                        # Windows 解压即用版（完整应用目录）
│   ├── textdiff.exe
│   ├── chrome-sandbox
│   ├── resources/
│   │   └── app.asar
│   ├── *.dll
│   └── ...
├── textdiff-1.1.0-setup.exe             # NSIS 安装包（x64 + ia32 合包，~174 MB）
├── textdiff-1.1.0-setup-x64.exe         # NSIS 安装包（仅 x64，~90 MB）
├── textdiff-1.1.0-setup-ia32.exe        # NSIS 安装包（仅 ia32，~85 MB）
├── textdiff-1.1.0-portable-x64.exe      # 便携版（免安装，~89 MB）
...
| 产物 | 格式 | 平台 | 说明 |
|------|------|------|------|
| `win-unpacked/` | 目录 | Windows x64 | 解压即用，无需安装 |
| `textdiff-x.x.x-setup.exe` | NSIS | Windows x64+ia32 | 双架构合一安装包 |
| `textdiff-x.x.x-setup-x64.exe` | NSIS | Windows x64 | 64 位安装包 |
| `textdiff-x.x.x-setup-ia32.exe` | NSIS | Windows ia32 | 32 位安装包 |
| `textdiff-x.x.x-portable-x64.exe` | Portable | Windows x64 | 免安装便携版 |
